...
def select_action(q_net, state, epsilon, track_direction):
    ...
    if random.random() < epsilon:
        ...
        if track_direction > 0.2:  # track curves right
            if random.random() < 0.4:
                action[1] = 1  # right
        elif track_direction < -0.2:  # track curves left
            if random.random() < 0.4:
                action[0] = 1  # left
        else:
        ...
        with torch.no_grad():
            ...
            # Convert Q-values to binary actions using sigmoid
            action_probs = torch.sigmoid(q_values.squeeze())
            action = [int(prob > 0.5) for prob in action_probs]
        return action
...
def main():
    ...
    # FIXED: Correct QNetwork initialization
                ...
                while True:
                    ...
                    try:
                        ...
                        if "crash" in events or "stuck" in events or stuck_counter > STUCK_THRESHOLD:
                            ...
                            prev_frame = None
                            prev_mask = None
                            time.sleep(1.0)  # Give time for restart
