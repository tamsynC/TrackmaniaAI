...
def reward_from_events(events, episode_length, max_episode_length, track_direction, car_speed=0, progress_delta=0, consecutive_checkpoints=0, car_center=None, mask=None):
    ...
    if "finish" in events:
        ...
        base_checkpoint_reward = 75  # Reduced from 6000
        consecutive_bonus = consecutive_checkpoints * 75  # Much smaller
    ...
    if car_speed > 0:
        speed_reward = min(car_speed, 5)  # Was * 2, max 20
        reward += speed_reward
...
#positive means on left side of track
# negative means on right side of track
