...
def reward_from_events(events, episode_length, max_episode_length, track_direction, car_speed=0, progress_delta=0, consecutive_checkpoints=0, car_center=None, mask=None):
    ...
    # MAJOR ISSUE FIX: Much smaller base rewards to reduce noise
    if "finish" in events:
        reward += 1000  # Reduced from 100000
        print(f"🏆 RACE COMPLETED!")
    
    elif "checkpoint" in events:
        base_checkpoint_reward = 100  # Reduced from 6000
        consecutive_bonus = min(consecutive_checkpoints * 5, 25)  # Much smaller
        reward += base_checkpoint_reward + consecutive_bonus
        print(f"✅ Checkpoint! Reward: {base_checkpoint_reward + consecutive_bonus}")
    
    # Smaller penalties to reduce noise
    if "crash" in events:
        reward -= 50  # Reduced from -1500
    
    if "stuck" in events:
        reward -= 30  # Reduced from -1500
    
    if "out_of_bounds" in events:
        reward -= 40  # Reduced from -2000
    
    if "checkpoint_timeout" in events:
        reward -= 100  # Reduced from -500
    
    # CRITICAL: Much smaller continuous rewards
    # Time alive reward - MUCH smaller
    time_alive_reward = 0.1  # Was episode_length * 0.3, now tiny
    reward += time_alive_reward
    
    # Track direction reward - smaller
    direction_reward = abs(track_direction) * 2  # Was * 8
    reward += direction_reward
    
    # Speed reward - smaller
    if car_speed > 0:
        speed_reward = min(car_speed * 0.5, 5)  # Was * 2, max 20
        reward += speed_reward
    
    # Small consistency bonus
    if not any(event in events for event in ["crash", "stuck", "out_of_bounds"]):
        consistency_bonus = 2  # Was 10
        reward += consistency_bonus
    
    # Remove the center-of-track calculation - it's too noisy
...
def select_action(q_net, state, epsilon, track_direction):
    if random.random() < epsilon:
        ...
        elif random.random() < 0.2:  # 20% chance to brake/reverse
            action_idx[3] = 1  # Backward/brake
            
        # Sometimes do nothing (coast)
        ...
        with torch.no_grad():
            ...
            action_probs = torch.sigmoid(q_values.squeeze())  # Each action 0-1 probability
            
            # Apply thresholds for each action
            action_idx = [
                (action_probs[0] > 0.4).int().item(),  # Left - lower threshold
                (action_probs[1] > 0.4).int().item(),  # Right - lower threshold  
                (action_probs[2] > 0.3).int().item(),  # Forward - even lower threshold
                (action_probs[3] > 0.6).int().item(),  # Backward - higher threshold
            ]
            ...
            for episode in range(total_episodes):
                ...
                while True:
                    ...
                    try:
                        ...
                        time.sleep(1 / 30)  # Slower to see action effect
