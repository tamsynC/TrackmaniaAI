...
GAMMA = 0.99
LEARNING_RATE = 1e-5
REPLAY_BUFFER_CAPACITY = 5500    #20gb ram, 115gb swap
...
turn_start_time = {'left': None, 'right': None}
turn_duration_threshold = 0.5  # 0.5 seconds
turn_punishment_per_frame = -5  # Punishment per frame after threshold
...
def create_enhanced_state(resized_frame, mask, car_center, track_direction, car_speed, prev_state=None):
    """Create enhanced state with additional information channels"""
    
    # Base RGB state
    base_state = preprocess_frame(resized_frame)  # Shape: (3, 512, 512)
    
    # Create additional information channels
    height, width = INPUT_SIZE
    
    # Channel 4: Track direction heatmap
    direction_channel = np.full((height, width), track_direction, dtype=np.float32)
    
    # Channel 5: Speed information
    speed_normalized = min(car_speed / 100.0, 1.0)  # Normalize speed
    speed_channel = np.full((height, width), speed_normalized, dtype=np.float32)
    
    # Channel 6: Car position heatmap
    car_channel = np.zeros((height, width), dtype=np.float32)
    if car_center is not None:
        # Create a small heatmap around car position
        x, y = car_center
        if 0 <= x < width and 0 <= y < height:
            car_channel[max(0, y-10):min(height, y+10), 
                       max(0, x-10):min(width, x+10)] = 1.0
    
    # Channel 7: Segmentation mask as additional info
    # Convert mask to normalized float
...
def reward_from_events(events, episode_length, max_episode_length, track_direction, 
                      ...
                      mask=None, current_time=None, crash_penalty=0, last_checkpoint_time=None, action_idx=[0,0,0,0]):
    reward = 0
    
    # 1. MAJOR REWARDS - Race completion
    if "finish" in events:
        base_finish_reward = 2000
        # Bonus for fast completion
        if episode_length < 1000:
            speed_bonus = (1000 - episode_length) * 2
            reward += base_finish_reward + speed_bonus
            print(f"🏆 RACE COMPLETED! +{base_finish_reward + speed_bonus}")
        else:
            reward += base_finish_reward
            print(f"🏆 RACE COMPLETED! +{base_finish_reward}")
    
    # 2. CHECKPOINT REWARDS - Progress tracking
    elif "checkpoint" in events:
        base_checkpoint_reward = 200 + (consecutive_checkpoints * 50)  # Instead of 100
        
        # Consecutive checkpoint bonus
        consecutive_bonus = min(consecutive_checkpoints * 20, 200)  # Cap at 200
        
        # Time-based bonus (reward faster checkpoint reaching)
        time_bonus = 0
        if last_checkpoint_time is not None:
            if last_checkpoint_time < 3.0:  # Very fast
                time_bonus = 100
                print("⚡ LIGHTNING FAST checkpoint!")
            elif last_checkpoint_time < 5.0:  # Fast
                time_bonus = 50
                print("🚀 FAST checkpoint!")
            elif last_checkpoint_time < 8.0:  # Good
                time_bonus = 25
                print("👍 GOOD checkpoint time!")
        
        total_checkpoint_reward = base_checkpoint_reward + consecutive_bonus + time_bonus
        reward += total_checkpoint_reward
        print(f"✅ Checkpoint: +{total_checkpoint_reward} (base:{base_checkpoint_reward} + streak:{consecutive_bonus} + time:{time_bonus})")
    
    # 3. PENALTIES - Discourage bad behavior
    if "crash" in events:
        # Reduce crash penalty as player gets better
        crash_penalty = max(50, 100 - consecutive_checkpoints * 10)    # Instead of 150
        reward -= crash_penalty
        print(f"💥 Crash penalty: -{crash_penalty}")
    
    if "stuck" in events:
        stuck_penalty = max(30, 80 - consecutive_checkpoints * 5)
        reward -= stuck_penalty
        print(f"🚫 Stuck penalty: -{stuck_penalty}")
    
    if "out_of_bounds" in events:
        oob_penalty = max(40, 100 - consecutive_checkpoints * 8)
        reward -= oob_penalty
        print(f"🚫 Out of bounds: -{oob_penalty}")
    
    if "checkpoint_timeout" in events:
        reward -= 20
        print("⏰ Checkpoint timeout: -20")
    
    # 4. CONTINUOUS REWARDS - Small incentives for good behavior
    
    # Direction alignment reward - encourage following track
    if abs(track_direction) < 0.5:  # On relatively straight sections
        direction_reward = 2
        reward += direction_reward
    elif abs(track_direction) > 0.8:  # Sharp turns
        # Small penalty for not slowing down on sharp turns
        if car_speed > 80:
            reward -= 1
    
    # Speed maintenance reward
    if car_speed > 20 and car_speed < 120:  # Good speed range
        speed_reward = 1
        reward += speed_reward
    elif car_speed < 5:  # Too slow
        reward -= 2
    
    # Checkpoint approach reward
    if checkpoint_coverage > 0.001:  # Checkpoint visible
        approach_reward = min(checkpoint_coverage * 100, 5)  # Max 5 points
        reward += approach_reward
    
    # 5. STREAK BONUSES - Reward consistency
    if consecutive_checkpoints > 0:
        consistency_bonus = min(consecutive_checkpoints * 0.5, 5)  # Small continuous bonus
        reward += consistency_bonus
    if action_idx is not None and current_time is not None:
        turn_punishment = detect_prolonged_turning(action_idx, current_time)
        if turn_punishment < 0:
            reward += turn_punishment
            left_duration = current_time - turn_start_time['left'] if turn_start_time['left'] else 0
            right_duration = current_time - turn_start_time['right'] if turn_start_time['right'] else 0
            max_duration = max(left_duration, right_duration)
            print(f"🔄 Prolonged turning penalty: {turn_punishment} (duration: {max_duration:.2f}s)")
    print(reward)
    return reward
...
class QNetwork(nn.Module):
    def __init__(self, input_shape, num_actions):
        ...
        self.conv_layers = nn.Sequential(
            ...
            # Fourth block - Added for better feature extraction
...
def restart_track():
    ...
    force_release_all_keys()
    time.sleep(2)
def select_action(q_net, state, epsilon, track_direction, car_speed=0, device='cuda'):
    if random.random() < epsilon:
        ...
        turn_prob = min(max(abs(track_direction), 0.1), 0.25)  
        ...
        with torch.no_grad():
            ...
            action_idx = [
                int(action_probs[0].item() > 0.4),  
                int(action_probs[1].item() > 0.4),  
                int(action_probs[2].item() > 0.1),  
                int(action_probs[3].item() > 0.8),  
            ]
            # Prevent conflicting actions
            if action_idx[0] == 1 and action_idx[1] == 1:
                if action_probs[0] > action_probs[1]:
                    action_idx[1] = 0
                else:
                    action_idx[0] = 0
                    
            if action_idx[2] == 1 and action_idx[3] == 1:
                if action_probs[2] > action_probs[3]:
                    action_idx[3] = 0
                else:
                    action_idx[2] = 0
        return action_idx
...
def reset_turn_tracking():
    """Reset turn tracking when episode starts/restarts"""
    global turn_start_time
    turn_start_time = {'left': None, 'right': None}

def detect_prolonged_turning(action_idx, current_time):
    """
    Detect if the agent has been turning in one direction for too long
    Returns punishment value if turning too long, 0 otherwise
    """
    global turn_start_time
    
    left_pressed = action_idx[0] == 1   # 'a' key
    right_pressed = action_idx[1] == 1  # 'd' key
    
    punishment = 0
    
    # Track left turn duration
    if left_pressed:
        if turn_start_time['left'] is None:
            turn_start_time['left'] = current_time
        else:
            duration = current_time - turn_start_time['left']
            if duration > turn_duration_threshold:
                punishment += turn_punishment_per_frame
                if duration > 1.0:  # Extra punishment for very long turns
                    punishment += turn_punishment_per_frame
    else:
        turn_start_time['left'] = None
    
    # Track right turn duration
    if right_pressed:
        if turn_start_time['right'] is None:
            turn_start_time['right'] = current_time
        else:
            duration = current_time - turn_start_time['right']
            if duration > turn_duration_threshold:
                punishment += turn_punishment_per_frame
                if duration > 1.0:  # Extra punishment for very long turns
                    punishment += turn_punishment_per_frame
    else:
        turn_start_time['right'] = None
...
def main():
    ...
    target_net = QNetwork((7, INPUT_SIZE[0], INPUT_SIZE[1]), NUM_ACTIONS).to(device)
            ...
            for episode in range(total_episodes):
                ...
                while True:
                    ...
                    try:
                        ...
                        action_idx = select_action(q_net, state, epsilon, track_direction, car_speed)
                        # action_idx = [0,1,1,0]
                        ...
                        try:
                            ...
                            next_state = create_enhanced_state(
                                resized_frame=resized_frame_next,
                                mask=mask_next,
                                car_center=car_center,
                                track_direction=track_direction,
                                car_speed=car_speed
                        ...
                        if prev_frame is not None:
                            ...
                            reward = reward_from_events(
                                    ...
                                    current_time, crash_penalty, last_checkpoint_time, action_idx
                                )
                            ...
                            prev_state = create_enhanced_state(
                                resized_frame=resized_prev_frame,
                                mask=prev_mask,
                                car_center=prev_car_pos,
                                track_direction=track_direction,
                                car_speed=car_speed
                            )
