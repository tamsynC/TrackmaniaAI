...
#     is_crash = track_coverage_ratio < 0.17
...
def detect_track_coverage_crash(mask, duration=1.5, check_interval=0.01, threshold=0.2):

    start_time = time.time()
    sustained_crash = True
    ...
    while time.time() - start_time < duration:
        ...
        if coverage >= threshold:
            sustained_crash = False
            break

        time.sleep(check_interval)

    if sustained_crash:
        print(f"🚨 Sustained track coverage crash detected (under {threshold*100:.1f}%) for {duration}s!")

    return sustained_crash
